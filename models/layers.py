...
def disp_to_depth(disp: Tensor, min_depth: float, max_depth: float) -> Tensor:
    """
...
class Conv3x3(Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, use_refl: bool = True,
    ):
        super().__init__()
        if use_refl:
...
class Project3D(nn.Module):
    ...
    """
    def __init__(
        self, batch_size: int, height: int, width: int, eps: float = 1e-7,
    ):
        super().__init__()
        self.batch_size = batch_size
        self.height = height
